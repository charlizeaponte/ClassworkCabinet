...
public class editDistance {
    ...
    public static void main(String[] args) {
         ...
         */
        // Base cases
        // DUNCAN: Added a simple base case 0,0
        d[0][0] = 0; // Done when both strings have no characters in them!

        // DUNCAN: Loop goes from 1 to length (instead of 0 to length)
        for (int i = 1; i <= input1_length; i++) {
            d[i][0] = i * 2; // Cost of deletion
            methods[i][0] = "Delete " + input1.charAt(i - 1);
        }

        // DUNCAN: Same for this loop.
        for (int j = 1; j <= input2_length; j++) {
            d[0][j] = j * 2; // Cost of insertion
            if (j > 0)
                methods[0][j] = "Insert " + input2.charAt(j - 1);
        }
        ...
        for (int i = input1_length, j = input2_length; i > 0 || j > 0;) {
            ...
            if (op != null) {
                if (op.startsWith("Replace")) {
                    ...
                    System.out.println(result + ": " + op);
                    // DUNCAN: Not sure why you printed it with inserted character at end.
